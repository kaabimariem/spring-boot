...
@Component({
  ...
  template: `
    <div class="page-container">
      <div class="header-section">
        ...
        <button class="btn-add" routerLink="/clients/new">
        ...
        <table class="modern-table">
          ...
          <tbody>
            <tr *ngFor="let client of clients" class="table-row">
              ...
              <td>{{ client.email }}</td>
              <td>{{ client.phone }}</td>
              <td class="address-col text-truncate">{{ client.address }}</td>
              <td class="actions">
                <button class="action-btn edit" title="Modifier" [routerLink]="['/clients/edit', client.id]">✏️</button>
                <button class="action-btn delete" title="Supprimer" (click)="onDelete(client.id)">🗑️</button>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .avatar { width: 32px; height: 32px; background: #3b82f6; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; }
    ...
    if (confirm('Voulez-vous vraiment supprimer ce client ?')) {
      this.clientService.deleteClient(id).subscribe({
        ...
        error: (err) => {
          ...
          alert("Impossible de supprimer le client. Il est peut-être lié à des factures.");
        }
      });
    }
  }
